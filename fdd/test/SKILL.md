...
```
fdd-fix-pm 出复现步骤+验收标准
    └── Test 写复现用例（串行，不Review）→ Dev 诊断根因+修复 → 全绿 → Test 扩展同类场景 → Review → Dev 修复 → 全绿交付

PM 厘清现象 → Test 转成可执行复现用例 → Dev 读代码定位根因并修复 → Test 扩展同类 → Review ↻ → Dev 修复到全绿
```
...
```
读 plans/ACTIVE.md → 确认 bug 任务 → 读 PM 确认的复现步骤和验收标准
```

### 2. 写复现用例（第一步，串行，不 Review）

根据 PM 确认的复现步骤写一个**必须失败的测试**：
...
更新 ACTIVE 为"复现就绪"。告知用户：复现用例已写好，让 fdd-fix-dev 诊断根因并修复。

### 4. 扩展同类场景（Dev 修复后）

Dev 修复完成全绿后，Test 扩展测试覆盖同类可能受影响的场景：
...
Bug 复现：写完后更新 ACTIVE 为"复现就绪"，移交 Dev。
Bug 扩展：Review 判决 ✅ 后，更新 ACTIVE 为"测试就绪"。
