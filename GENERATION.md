...
- **Commit SHA**: `7016ab3355b45ef47b122211de94d2495af9d3fc`
- **Date**: 2026-09-16
- **Commit**: feat(secret-handoff): broker credentials without exposing secrets
...
│   ├── SKILL.md                # Main skill file
│   ├── agents/openai.yaml      # UI metadata (display name / short description)
│   └── references/             # 1 file: bitwarden.md — Bitwarden CLI / SSH Agent handoff
├── teach/                       # Active
│   ├── SKILL.md                # Main skill file
...
| `secret-handoff` | 安全地向 agent 交接密码、API token、SSH 与 Vault 凭据——broker 优先、最小权限、脱敏、一次性文件与清理；支持 Bitwarden、1Password、macOS Keychain、SSH Agent。 | SKILL.md, agents/openai.yaml, references/ (1) |
...
git diff --name-only 7016ab3..HEAD -- '*/SKILL.md'

# See full diff of skill changes
git diff 7016ab3..HEAD -- '*/SKILL.md'

# See commit log for skills
git log --oneline 7016ab3..HEAD -- '*/SKILL.md'
```
