---
name: api-design
description: API 设计指南——渐进式增强、框架无关、DX 优先、原子化。当设计 API、评审接口、重构签名，或讨论函数/CLI/REST/GraphQL 对外形态时使用。
---
...
6. **向后兼容**（重构场景）— 旧调用方迁移路径、兼容垫片、共存期。**完成标志**：每个旧调用点都有迁移方案。
7. **输出方案** — 按[输出格式](#输出格式)呈现。**完成标志**：输出格式中所有必填段均已填充，调用示例在接口定义之前。
