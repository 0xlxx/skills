...
**Agent B（实现者）**执行 Phase 3→4。读形式化分析文档和已有测试，不读上游源码（避免被参考实现思维污染）。实现必须让测试变绿。
...
6. **TS adaptation notes** — Java/C → TS mapping decisions (e.g. "Map<LNode, T> instead of T[] indexed by node.id")
