...
Use the template in the main skill (§3) — one `mktemp -d` per task, never a shared fixed path. A fixed `~/.cache/.../bw_session` invites races, stale sessions, and symlink tricks; a fresh directory per task removes all three. Cleanup: truncate + unlink the file, remove the directory, and verify the **exact path** afterwards (`test ! -e '<dir>'`, not `$DIR` — a later tool call has no such variable).
